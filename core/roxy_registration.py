...
def _human_click(driver, el, *, label: str = "") -> None:
    ...
    """
    if el is None:
        raise RuntimeError(f"点击目标为空: {label or 'unknown'}")
...
def _human_type_text(driver, el, value: str, *, clear: bool = True, blur: bool = True) -> None:
    """按人工节奏输入，并保留清空、失焦和 WebDriver 键盘事件语义。"""
    if el is None:
        raise RuntimeError("输入目标为空")
    try:
        _human_scroll_to(driver, el)
        try:
        ...
        driver.execute_script(r"""
        const el = arguments[0], value = String(arguments[1]), blur = !!arguments[2];
...
def _submit_nearest_form_for_active_input(driver) -> bool:
    ...
    return {ok:true, reason:safe[0].isPrimarySubmit ? 'primary_submit' : 'safe_submit', target:safe[0].el, targetAttrs:safe[0].attrs.slice(0,160), primary:safe[0].isPrimarySubmit};
    """) or {}
    if result.get("ok"):
...
def _type_otp(driver, code: str) -> None:
    ...
    for selector in [
        ...
        if len(els) == 1:
            _human_type_text(driver, els[0], code, clear=True, blur=False)
            return
    ...
    if len(numeric_boxes) >= len(code):
        for e, ch in zip(numeric_boxes, code):
            _human_type_text(driver, e, ch, clear=True, blur=False)
        return
...
def _set_element_value(driver, el, value: str) -> None:
    ...
    if el.__class__.__name__ != "CloakElement":
        _human_type_text(driver, el, str(value))
        return
    driver.execute_script(r"""
    const el = arguments[0];
...
def _submit_existing_login_password(driver, password: str, timeout: int = 30) -> str:
    ...
    _human_type_text(driver, password_input, password, clear=True, blur=False)
    target = driver.execute_script(r"""
    ...
    _human_click(driver, target, label="existing_password_submit")
...
def _click_if_enabled_submit(driver) -> bool:
    """提交资料页：优先人工化点击可见按钮，仅在无按钮时 requestSubmit。"""
    try:
        from selenium.webdriver.common.by import By
        ...
        for form in forms:
            ...
            if submitters:
                _human_click(driver, submitters[0], label="profile_form_submit")
                return True
        ...
        if submitters:
            _human_click(driver, submitters[0], label="profile_submit")
            return True

        buttons = [el for el in driver.find_elements(By.CSS_SELECTOR, "button:not([disabled])") if _visible(el)]
        if len(buttons) == 1:
            _human_click(driver, buttons[0], label="profile_single_button")
            return True
