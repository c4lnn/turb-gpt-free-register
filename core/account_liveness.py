...
def _network_preflight_with_retry(email: str, proxy: str | None, max_attempts: int = 4) -> tuple[BrowserSession, str]:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        fingerprint_summary_text = getattr(session, "fingerprint_summary_text", None)
        if callable(fingerprint_summary_text):
            logger.info("[查活] 指纹摘要：%s", fingerprint_summary_text())
        try:
